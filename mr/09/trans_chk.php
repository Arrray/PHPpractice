...
if($_FILES[lyric][size] > 0){
	...
	if(!($a_rst == false)){
		$quesql = "select tb_music_id,tb_music_counts from tb_music_user where tb_music_id = ".$_SESSION[id];
		$querst = $result->getRows($quesql,$conn);
		$count = $querst[0][tb_music_counts];
		$count += 1;
		$addsql = "update tb_music_user set tb_music_counts = ".$count." where tb_music_id = ".$_SESSION[id];
		$addrst = $result->indeup($addsql,$conn);
		if(!($addrst == false)){
		$_SESSION[countes] += 1;
		echo "<script>alert('添加成功');window.location.href='index.php?lmbs=上传音乐';</script>";
		exit();
		}
	}else{
