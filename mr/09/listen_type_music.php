...
//$sql = "select id,style,name,actor,remark,address from tb_video order by id";
$sql = "select id,style,name,actor,remark,address from tb_video where style='".$_GET[music_type]."' order by id";
...
if(empty($_GET[pages])==true || is_numeric($_GET[pages])==false){
	...
	$l_sqlstr="select id,style,name,actor,remark,address from tb_video where style='".$_GET[music_type]."' order by id";
...
$smarty->display("listen_type_music.tpl");
